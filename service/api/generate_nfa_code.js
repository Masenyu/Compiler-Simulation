  ...
  if(existOfNull!=-1) {
    ...
    for(var i=0;i<NFAdata.transitionTable.length;i++){
      ...
      if(i+1==NFAdata.transitionTable.length){
        ...
        temp_str+=" ] //状态 "+i+"在输入字符"+NFAdata.alphabet.toString()+"对应的变更状态 (空表示不存在) \n"
      }else{
        for(var j=0;j<NFAdata.transitionTable[i].length;j++){
          var temp_str1="[";
          if(j+1==NFAdata.transitionTable[i].length){
    ...
    code_str+="];\n\n"
    code_str+="function simulateStateMachine(input){\n" +
      "  var indexOfNull = -1;\n" +
    ...
    //构建字母表  ！！！！！
    var code_str="const int kNumOfStates = "+NFAdata.transitionTable.length+"; //状态机中状态的总数\n" ;
    code_str+="const int kNumOfAlphabet = "+NFAdata.alphabet.length+"; //状态机中使用到的字符的总数\n\n" ;
    code_str+="char alphabet[ kNumOfAlphabet ] ={";
    for(var i=0;i<NFAdata.alphabet.length;i++){
      if(i==0){code_str+=NFAdata.alphabet[i]}else{
        code_str+=','+NFAdata.alphabet[i]
      }
    }
    code_str+="};\n\n"
    ...
    code_str+=" int state = 0; //用来保存状态机数的变量0为开始状态，初始化为0\n"
    code_str+=" for (int i = 0 ; i < input.length ; i++ ){\n"
    code_str+="   int indexOfInput = -1 ; //记录当前输入字符在字母表中的下标，如果输入字符不在字母表中，返回-1，反之返回字符在字母表中对应的下标\n"
    ...
    code_str+="   }else{\n"
    code_str+="     state = kTransitionTable[state][indexOfInput] ; //当前状态在输入当前字符会出现的状态变更\n"
    code_str+="   }\n"
    code_str+=" }\n"
    code_str+=" return kAcceptTable[state]; //判断最终状态是否为接受状态 为接受状态则返回true接受该字符串，反之返回false  \n"
    code_str+="}\n"
    ...
    //
    // var str_result="const int kNumStates = "+NFAdata.transitionTable.length+"\n";
    // str_result+="const int kNumSymbols = "+NFAdata.alphabet.length+"\n\n";
    // str_result+="string alphabet[ kNumSymbols ]={\n"
    // for(var i=0;i<NFAdata.alphabet.length;i++){
    //   if(i==0){str_result+="\'"+NFAdata.alphabet[i]+'\''}else{str_result+=",\'"+NFAdata.alphabet[i]+ "\'"}
    // }
    // str_result+="\n}\n"
    //
    // str_result+="\n"
    // str_result+="vector \< Integer \> kTransitionTable[kNumStates][kNumSymbols]={\n" +
    //   "//alphabet:"+NFAdata.alphabet+"\n"
    //
    // for(var j=0 ;j<NFAdata.transitionTable.length;j++){
    //
    //   var str=""
    //   for(var i=0;i<NFAdata.alphabet.length;i++){
    //     var str_state=""
    //     for(var k=0;k<NFAdata.transitionTable[j][i].length;k++){
    //       if(k==0){
    //         str_state+=NFAdata.transitionTable[j][i][k]
    //       }else{
    //         str_state+=","+NFAdata.transitionTable[j][i][k]
    //       }
    //     }
    //     NFAdata.transitionTable[j][i].length
    //     if(i==0){str+="{"+ str_state +"}"}else{str+=",{"+ str_state +"}"}
    //   }
    //
    //   if(j+1!=NFAdata.transitionTable.length)
    //   {str="{ "+str+" },\n"}else{
    //     str="{ "+str+" }\n"
    //   }
    //
    ...
    // str_result+="};\n"
    //
    // str_result+="bool kAcceptTable[kNumStates]={\n"
    // for(var i=0 ;i<NFAdata.transitionTable.length;i++){
    //   var temp=false
    //   for(var j=0;j<NFAdata.acceptStateList.length;j++){
    //     if(NFAdata.acceptStateList[j].state==i){temp=true;break}
    //   }
    //   if(i+1!=NFAdata.acceptStateList.length){
    //     if(temp){str_result+="true,//状态"+i+"为接受态\n"}else{str_result+="false,//状态"+i+"\n"}
    //   }else{
    //     if(temp){str_result+="true//状态"+i+"为接受态\n"}else{str_result+="false//状态"+i+"\n"}
    //   }
    //
